...
<div class="report-panel overflow-auto"><table class="report-table"><thead><tr><th>Sr</th><th>Transport</th><th>LR Date</th><th>LR No</th><th>Vehicle Type</th><th>Lorry</th><th>SO / Ref</th><th>From</th><th>To</th><th>Supplier</th><th>Supp. Freight</th><th>Supp. Advance</th><th>Supp. Paid</th><th>Supp. Balance</th><th>Customer</th><th>Cust. Freight</th><th>Paid</th><th>Balance</th><th>Hamali L</th><th>Hamali U</th><th>Other Charges</th><th>Profit</th><th>Bill No</th><th>GST</th><th>Remarks</th><th class="no-print">Bill</th></tr></thead><tbody>
@forelse($vouchers as $v)
@php
    $sp=(float)($v->supplier_payment_total??0);
    $cp=(float)($v->customer_paid_total??0);
    $sb=max(0,(float)$v->supplier_freight-(float)$v->supplier_advance-$sp);
    $customerAmount=(float)$v->customer_freight+(float)$v->hamali_loading+(float)$v->hamali_unloading+(float)$v->other_charges;
    $cb=max(0,$customerAmount+(float)$v->gst-$cp);
    $profit=(float)$v->customer_freight-(float)$v->supplier_freight;
@endphp
<tr><td>{{ $v->sr_no }}</td><td class="font-bold">{{ $v->transportCompany?->name }}</td><td>{{ optional($v->lr_date)->format('d-m-Y') }}</td><td>{{ $v->lr_no?:'-' }}</td><td>{{ $v->vehicleType?->name?:'-' }}</td><td>{{ $v->lorry_no?:'-' }}</td><td>{{ $v->so_ref_no?:'-' }}</td><td>{{ $v->from_place?:'-' }}</td><td>{{ $v->to_place?:'-' }}</td><td>{{ $v->supplier?->name?:'-' }}</td><td class="num">₹{{ number_format((float)$v->supplier_freight,2) }}</td><td class="num">₹{{ number_format((float)$v->supplier_advance,2) }}</td><td class="num">₹{{ number_format($sp,2) }}</td><td class="num font-bold">₹{{ number_format($sb,2) }}</td><td>{{ $v->customer?->name }}</td><td class="num">₹{{ number_format((float)$v->customer_freight,2) }}</td><td class="num">₹{{ number_format($cp,2) }}</td><td class="num font-bold">₹{{ number_format($cb,2) }}</td><td class="num">₹{{ number_format((float)$v->hamali_loading,2) }}</td><td class="num">₹{{ number_format((float)$v->hamali_unloading,2) }}</td><td class="num">₹{{ number_format((float)$v->other_charges,2) }}</td><td class="num font-black {{ $profit>=0?'positive':'negative' }}">₹{{ number_format($profit,2) }}</td><td>{{ $v->bill_no?:'-' }}</td><td class="num">₹{{ number_format((float)$v->gst,2) }}</td><td>{{ $v->remarks?:'-' }}</td><td class="no-print"><a target="_blank" href="{{ route('reports.customer-bill',$v) }}" class="font-bold text-blue-700 underline">Print</a></td></tr>
@empty<tr><td colspan="26" class="py-10 text-center text-slate-400">No vouchers found for selected filters.</td></tr>@endforelse
