...
<div class="app-workspace">
    ...
    <div class="mt-5 border border-slate-200 bg-white shadow-sm">
        ...
        @forelse($recent as $row)
            @php
                $paid=(float)($row->customer_paid_total??0);
                $customerAmount=(float)$row->customer_freight+(float)$row->hamali_loading+(float)$row->hamali_unloading+(float)$row->other_charges+(float)$row->gst;
                $bal=$customerAmount-$paid;
            @endphp
