...
<div class="paper">
    ...
    <div class="invoice-gst center">GSTIN: {{ $companyGstNo ?: '-' }}</div>
    <div class="top-grid">
        <div>
    ...
    <div class="terms-sign"><div class="terms"><h4>TERMS OF PAYMENT</h4><ol><li>Please do not deduct any amount from the bill without our consent.</li><li>Any dispute is subject to local jurisdiction only.</li><li>Payment should quote the invoice number.</li></ol>@if($invoice->remarks)<div style="margin-top:8px"><b>Remarks:</b> {{ $invoice->remarks }}</div>@endif</div><div class="signature"><div class="bold">For {{ $companyName }}</div><div class="bold">Authorised Signatory</div></div></div>
    <div class="bank"><b>BANK DETAILS</b>
        @if($companyBank)
            <br><b>Bank Name:</b> {{ $companyBank->name }}
            <br><b>Holder Name:</b> {{ $companyBank->account_holder_name ?: $companyName }}
            <br><b>Account Number:</b> {{ $companyBank->account_number ?: '-' }}
            <br><b>IFSC Code:</b> {{ $companyBank->ifsc_code ?: '-' }}
        @else
            <br><span>Default bank is not configured for {{ $companyName }}.</span>
        @endif
    </div>
