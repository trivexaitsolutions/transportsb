{{-- Company Master is the only source for the Voucher Company selector. --}}
@extends('layouts.app')
...
document.addEventListener('DOMContentLoaded',()=>{
    ...
    const fieldOrder=['company_id','lr_date','lr_no','vehicle_type_id','lorry_number','sales_order_id','supplier_id','supplier_freight','advance_paid','advance_mode','hamali_loading','hamali_unloading','other_charges','remarks'];
    const selectorDefs={company_id:{type:'companies',title:'Select Company',label:'company_name'},vehicle_type_id:{type:'vehicle-types',title:'Select Vehicle Type',label:'vehicle_type_name'},sales_order_id:{title:'Select Sales Order',label:'so_number',so:true},supplier_id:{type:'suppliers',title:'Select Supplier / Transporter',label:'supplier_name'}};
    ...
    function activeRow(r){return !!(r.id||r.company_id||r.lr_no||r.vehicle_type_id||r.lorry_number||r.sales_order_id||r.supplier_id||num(r.supplier_freight)||num(r.advance_paid)||r.advance_mode||num(r.hamali_loading)||num(r.hamali_unloading)||num(r.other_charges)||r.remarks||r.description);}
