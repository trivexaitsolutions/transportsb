...
.inline-add-btn{width:22px;height:33px;border-left:1px solid #cbd5e1;background:#f1f5f9;color:#047857;font-weight:900;outline:none}.inline-add-btn:hover{background:#e2e8f0}.inline-add-btn:focus{box-shadow:inset 0 0 0 2px #059669}
</style>
@endpush
...
<div class="app-workspace voucher-workspace" id="voucherWorkspace">
    <div class="voucher-head">
        ...
        <div class="mt-2 flex flex-wrap gap-x-5 gap-y-1 text-[10px] font-bold text-slate-600">
            <span><b>From / To Date</b> Change dates to load trips</span><span><b>Enter on empty selector</b> Open selector</span><span><b>Enter on selected selector</b> Next cell</span><span><b>Backspace</b> Clear selection</span><span><b>← →</b> Previous / next</span><span><b>Ctrl+S</b> Save all rows</span><span><b>+ Button</b> Add Master Inline</span>
        </div>
    </div>
    ...
    <div class="voucher-grid-wrap" id="gridWrap">
        <table class="voucher-grid">
            <colgroup>
                <!-- Adjusted column widths to remove Description and fit Other Charges properly -->
                <col style="width:50px"><col style="width:135px"><col style="width:120px"><col style="width:115px"><col style="width:155px"><col style="width:135px"><col style="width:170px"><col style="width:190px"><col style="width:190px"><col style="width:125px"><col style="width:140px"><col style="width:125px"><col style="width:130px"><col style="width:125px"><col style="width:125px"><col style="width:125px"><col style="width:180px"><col style="width:70px">
            </colgroup>
            <thead><tr>
                <th>Sr No</th><th>Transport Name</th><th>LR Date</th><th>LR No</th><th>Vehicle Type</th><th>Lorry Number</th><th>SO No</th><th>Customer</th><th>Supplier / Transporter</th><th>Supplier Freight</th><th>Advance Paid</th><th>Balance</th><th>Supplier Payment</th><th>Hamali Loading</th><th>Hamali Unloading</th><th>Other Charges</th><th>Remarks</th><th>Delete</th>
            </tr></thead>
...
document.addEventListener('DOMContentLoaded',()=>{
    ...
    // Updated fieldOrder with correct sequence (removed description)
    ...
    // Add logic mapped to selectors
    const selectorDefs={transport_name_id:{type:'transport-names',title:'Select Transport Name',label:'transport_name'},vehicle_type_id:{type:'vehicle-types',title:'Select Vehicle Type',label:'vehicle_type_name',allowAdd:true},sales_order_id:{title:'Select Sales Order',label:'so_number',so:true},supplier_id:{type:'suppliers',title:'Select Supplier / Transporter',label:'supplier_name',allowAdd:true}};
    ...
    // Description field handling removed from row creation safely
    ...
    function activeRow(r){return !!(r.id||r.transport_name_id||r.lr_no||r.vehicle_type_id||r.lorry_number||r.sales_order_id||r.supplier_id||num(r.supplier_freight)||num(r.advance_paid)||num(r.hamali_loading)||num(r.hamali_unloading)||num(r.other_charges)||r.remarks);}
    ...
    // Updated selectorCell to include inline + button if allowAdd is true
    function selectorCell(i,field){const r=rows[i],d=selectorDefs[field],label=r[d.label]||'Select'; 
        let html=`<button type="button" data-row="${i}" data-field="${field}" data-grid-cell data-selector class="grid-button flex-1 ${r[field]?'':'empty'}">${esc(label)}</button>`;
        if(d.allowAdd){
            html=`<div class="flex items-center w-full h-full">${html}<button type="button" data-row="${i}" data-field="${field}" data-grid-cell data-inline-add title="Add New ${d.title}" class="inline-add-btn">+</button></div>`;
        }
        return html;
    }
    ...
    function openSelector(i,field,search='',forceAdd=false){
        const r=rows[i],d=selectorDefs[field],opener=cell(i,field);
        const opts={
            title:forceAdd?`Add ${d.title.replace('Select ','')}`:d.title,
            opener,search,allowAdd:!!d.allowAdd,forceAdd:forceAdd,
            onSelect:(item,meta)=>{
                r[field]=item.id;
                r[d.label]=item.label||item.name;
                if(field==='sales_order_id'){
                    r.so_number=item.so_number||item.name;r.customer_name=item.customer_name||'';r.from_location=item.from_location||'';r.to_location=item.to_location||'';r.per_trip_cost=num(item.per_trip_cost);
                }
                markDirty();render();setTimeout(()=>meta?.source==='quick-add'?focusCell(i,field,false):nextCell(i,field),25);
            }
        };
        if(d.so){
            opts.url=routes.soOptions+(r.id?('?include_id='+encodeURIComponent(r.sales_order_id||'')):'');opts.allowAdd=false;
        }else{
            opts.type=d.type;
        }
        MasterSelector.open(opts);
    }
    ...
    body.addEventListener('input',e=>{
        const el=e.target.closest('input[data-grid-cell]');if(!el)return;
        const i=Number(el.dataset.row),field=el.dataset.field;
        // UpperCase Lorry enforcement mapping on input
        if(field==='lorry_number') {
            const p = el.selectionStart;
            el.value=el.value.toUpperCase();
            try{ el.setSelectionRange(p,p); }catch(ex){}
        }
        rows[i][field]=el.value;recompute(rows[i]);markDirty();
        const bal=body.querySelector(`tr[data-row-index="${i}"] .computed`);if(bal)bal.textContent=money(rows[i].supplier_balance);updateTotals();
    });
    
    body.addEventListener('click',e=>{
        const del=e.target.closest('[data-row-delete]');if(del){selectedIndex=Number(del.dataset.rowDelete);deleteSelected();return;}
        const inlineAdd=e.target.closest('[data-inline-add]');if(inlineAdd){openSelector(Number(inlineAdd.dataset.row),inlineAdd.dataset.field,'',true);return;}
        const sel=e.target.closest('[data-selector]');if(sel){openSelector(Number(sel.dataset.row),sel.dataset.field);return;}
        const pay=e.target.closest('[data-payment]');if(pay){openPayment(Number(pay.dataset.row));}
    });
    
    body.addEventListener('keydown',e=>{
        const el=e.target.closest('[data-grid-cell]');if(!el)return;
        const i=Number(el.dataset.row),field=el.dataset.field,r=rows[i];
        if(e.key==='ArrowUp'){e.preventDefault();if(i===0)fromDate.focus();else vertical(i,field,-1);return;}
        if(e.key==='ArrowDown'){e.preventDefault();vertical(i,field,1);return;}
        if(e.key==='ArrowLeft'&&!(el instanceof HTMLInputElement&&el.type==='date')){if(el instanceof HTMLInputElement&&el.type==='text'&&el.selectionStart>0)return;e.preventDefault();prevCell(i,field);return;}
        if(e.key==='ArrowRight'&&!(el instanceof HTMLInputElement&&el.type==='date')){if(el instanceof HTMLInputElement&&el.type==='text'&&el.selectionStart<(el.value||'').length)return;e.preventDefault();nextCell(i,field);return;}
        if(e.key==='Backspace'&&el.matches('[data-selector]')){e.preventDefault();clearSelector(i,field);return;}
        if(e.key==='Backspace'&&el instanceof HTMLInputElement&&el.type==='date'){e.preventDefault();el.value='';r[field]='';markDirty();return;}
        if(el.matches('[data-selector]')&&e.key.length===1&&!e.ctrlKey&&!e.altKey&&!e.metaKey){e.preventDefault();openSelector(i,field,e.key);return;}
        if(e.key!=='Enter')return;
        e.preventDefault();
        if(el.matches('[data-selector]')){if(r[field])nextCell(i,field);else openSelector(i,field);return;}
        if(el.matches('[data-payment]')){nextCell(i,field);return;}
        if(el instanceof HTMLInputElement&&el.type==='date'&&!el.value){el.showPicker?.();return;}
        nextCell(i,field);
    });
    
    async function saveAll(){if(!fromDate.value||!toDate.value||toDate.value<fromDate.value){setStatus('Select a valid date range','error');fromDate.focus();return;}
        // Description field removed from payload safely.
        const payload=rows.filter(activeRow).map(r=>({id:r.id,lr_date:r.lr_date||fromDate.value,sales_order_id:r.sales_order_id,transport_name_id:r.transport_name_id,lr_no:r.lr_no||null,vehicle_type_id:r.vehicle_type_id,lorry_number:r.lorry_number||null,supplier_id:r.supplier_id,supplier_freight:num(r.supplier_freight),advance_paid:num(r.advance_paid),hamali_loading:num(r.hamali_loading),hamali_unloading:num(r.hamali_unloading),other_charges:num(r.other_charges),remarks:r.remarks||null}));
        setStatus('Saving...');
        try{
            const res=await fetch(routes.save,{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json','X-CSRF-TOKEN':csrf},body:JSON.stringify({from_date:fromDate.value,to_date:toDate.value,rows:payload})});
            const data=await res.json();
            if(!res.ok)throw new Error(data.message||Object.values(data.errors||{})[0]?.[0]||'Save failed');
            dirty=false;window.AppToast?.(data.message||'Voucher entries saved successfully.','success');
            window.location.href=data.redirect;
        }catch(err){setStatus(err.message||'Save failed','error');window.AppToast?.(err.message||'Save failed','error',5500);}
    }
    ...
    async function deleteSelected(){
        ...
        if(r.id){
            if(!confirm('Delete this voucher trip?'))return;
            try{
                const res=await fetch(routes.base+'/'+r.id,{method:'DELETE',headers:{Accept:'application/json','X-CSRF-TOKEN':csrf}});
                let data={};try{data=await res.json();}catch(_e){}
                if(!res.ok)throw new Error(data.message||'This voucher trip cannot be deleted.');
                rows.splice(selectedIndex,1);selectedIndex=Math.max(0,selectedIndex-1);render();markSaved();
                window.AppToast?.(data.message||'Voucher trip deleted.','success');
            }catch(err){
                window.AppToast?.(err.message||'This voucher trip cannot be deleted.','error',5500);
            }
            return;
        }
        rows.splice(selectedIndex,1);if(!rows.length)rows.push(blankRow());selectedIndex=Math.max(0,selectedIndex-1);render();markDirty();
    }
    ...
    async function loadRange(focusGrid=false){
        ...
        try{
            ...
            selectedIndex=0;dirty=false;render();markSaved('Entries loaded');
    ...
    fromDate.addEventListener('keydown',e=>{if(e.key==='Backspace'){e.preventDefault();fromDate.value='';return;}if(e.key==='Enter'){e.preventDefault();if(!fromDate.value){fromDate.showPicker?.();return;}toDate.focus();return;}});
    toDate.addEventListener('keydown',e=>{if(e.key==='Backspace'){e.preventDefault();toDate.value='';return;}if(e.key==='Enter'){e.preventDefault();if(!toDate.value){toDate.showPicker?.();return;}requestRange(true);return;}});
